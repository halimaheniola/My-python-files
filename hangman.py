...
def load_words():
    ...
    # line: string
    '''line = inFile.readline for py2'''
    content = inFile.read() 
    # wordlist: list of strings string.split(line) for py2
    wordlist = content.split() # split() has space as its default because
    #it is empty could be " " or ,. opp of split is join
    
    
    #print "  ", len(wordlist), "words loaded."
    return wordlist

def choose_word(wordlist): # we could use a different placeholder say list of words the impo
    #rtant thing to know is that wordlist  is in another scope performing a
...
# add a play function to the game and a scoring system that makes player
#to keep playing
# if y == z
#   return x
# return k
...
#   return x
# else
#   return k is a long way of doing this
